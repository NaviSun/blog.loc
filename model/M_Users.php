...
{	
	...
	public function Register($login, $password)
	{
		...
		$object = array('login' => $login, 'password' => $password);
	...
	{	
		...
		$query = sprintf($t, mysqli_real_escape_string($this->msql->link, $login));
		$result = $this->msql->Select($query);
		return $result[0];
	}
	...
	{		
		...
		$t = "SELECT * FROM users JOIN privs2roles using(id_role) JOIN privs USING (id_priv) WHERE id_user =  '%s'";	 
		$query = sprintf($t, mysqli_real_escape_string($this->msql->link, $id_user));
		$result = $this->msql->Select($query);
		$id_priv = $result[0]['id_priv'];
		
		var_dump((int)$id_priv);
		var_dump((int)$priv);
	...
	{	
		...
		$query = sprintf($t, mysqli_real_escape_string($this->msql->link, $sid));
		$result = $this->msql->Select($query);
	...
	private function GetSid()
	{
		...
		if ($sid != null)
		{
			...
			$t = "sid = '%s'";
			$where = sprintf($t, mysqli_real_escape_string($this->msql->link, $sid));
